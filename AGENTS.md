...
- 图表位于用量数据列表下方，默认显示最近一个月；右上角提供周、月、
  三个月和全部可用历史切换，设置中提供独立显示开关。
- 每行固定 `16` 个 `12 × 12 pt` 连续圆角方格，间距 `3 pt`；每个时间尺度
  都必须形成完整的 `16 × N` 网格，真实日期从右下角对齐。
- 左上角补齐格使用 `3.5%` 透明度的最浅填充、`0.75 pt` 虚线描边；补齐格
  不代表真实零用量，也不提供 Tooltip。
