...
灵动岛内容开发台永久保留于 [Prototypes/IslandTextConsole](Prototypes/IslandTextConsole/README.md)，
入口为该目录的 `Open Console.command`。后续涉及灵动岛文字、状态或布局时，可按需重建供用户手动验收。
它复用当前生产渲染器的隔离副本，不写生产数据；不得作为临时 QA 清理，也不得将模拟入口迁入正式应用。
源码和脚本进入版本管理，生成包与缓存保持忽略。使用和修改前阅读目录内的 `AGENTS.md`。
