...
稳定基线一致。App Store 适配工作另由 `docs/specs/README.md` 和对应唯一
规格约束，不得改写既有稳定发布事实。
...
4. 回到 `HANDOFF.md`，继续读取当前结论、验证状态、工作区边界和下一次
   迭代入口；
5. 如果当前分支存在 `docs/specs/README.md`，读取当前唯一规格；
6. 仅在任务涉及视觉验收历史时读取 `design-qa.md`；
...
- `VERSION_HISTORY.md`：当前最新版本指针、公开历史版本、各版本特性、
  撤回原因和替代版本；
- `docs/specs/README.md`：当前分支的唯一规格索引、Requirement 与交付状态；
- `docs/design/*.md`：具体产品、架构与发行适配规格；
...
### App Store 1.0.0 额度重置边界

用户已于 2026-08-05 明确授权 `codex/app-store-v1.0.0a` 移除额度重置操作。
该分支不得包含额度重置入口、详情页、确认层、设置开关、Demo 执行器或可用
重置次数的展示与 Widget 投影；可以继续显示只读的用量周期重置时间和普通
Credits 余额。下方额度重置视觉规则只描述 `0.3.1 Build 2` 直接分发稳定
基线，不得重新迁回 App Store `1.0.0`。
