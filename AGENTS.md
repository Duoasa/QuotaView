...
当前任务中明确要求改变产品方向，否则实现应保持与当前 0.1.5 Build 6
一致。
...
3. `HANDOFF.md`
4. `design-qa.md`
5. Figma 节点 `1:712`、`10:122`、`10:181`、`10:333`
6. 本文件中的通用设计规则
...
- 圆角必须与组件高度匹配。除明确的圆形按钮、胶囊或进度轨道外，圆角
  必须小于组件高度的一半。
- 状态标签固定为 `18 pt` 高、`6 pt` 连续圆角；不得恢复 `12 pt`
  圆角或任何会产生橄榄球形轮廓的写法。
...
|---|---:|
| 主面板和重置详情玻璃主体 | `21 pt` |
| 状态、订阅与 Demo 小标签 | `6 pt` |
| 进度条 | `6 pt` |
| 主面板 App Icon 容器 | `7.5 pt` |
| 重置入口、重置按钮、设置分组卡片 | `12 pt` |
| 最终确认卡片 | `26 pt` |
| 设置 Sidebar | `20 pt` |
| 设置窗口 | `36 pt` |
| 24 pt 功能按钮 | 圆形 |
...
- 深色可用色：`#00FF3F`；浅色可用文字色：`#149734`。
- 深色不可用色：`#FF453A`；浅色不可用文字色：`#B3261E`。
- 标签填充使用对应状态色 `20%`，由单个 `RoundedRectangle` 直接绘制。
- 描边为 `0.5 pt`；深色透明度 `12%`，浅色透明度 `50%`。
- 状态填充不得使用 `CIGaussianBlur`、外发光或投影，颜色不得超出
  `18 pt` 高、`6 pt` 圆角的边界。
...
- 重置入口卡片允许既有圆角 Core Animation `shadowPath`：
  黑色 `20%`、radius `15`；深色 offset `(0, 18)`，浅色 `(0, 6)`。
...
- 不增加第二层主面板玻璃或整面内层卡片。
- 不恢复状态标签的模糊背景、12 pt 圆角、外发光或胶囊形填充。
