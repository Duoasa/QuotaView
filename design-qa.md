...
2026-08-10 至 2026-08-11，产品所有者对真实运行中的候选构建进行了连续
手动验收，并明确选择当前结果作为 `0.3.3 (Build 3)` 正式版：

- 通过：周、月、三个月和总计均使用完整 `16 × N` 方格表，真实数据右下
  对齐，左上角使用虚线浅底占位格；
- 通过：切换周期时菜单顶部保持不动，下边缘收缩和扩展均不再发生上下跳动；
- 通过：Tooltip 延迟为 `0.5 s`，Token 数量使用紧凑 `K / M / B` 表达；
- 通过：数据格改为高区分度五级不透明单色；深色模式使用
  `16% → 80%` 白阶，浅色模式使用 `80% → 16%` 灰阶；占位格保留低透明度；
- 通过：设置中的 Token 活动图表显示/隐藏开关和默认最近一个月方向获得
  产品所有者确认。

本记录仅覆盖上述图表观感、周期切换和面板尺寸行为。浅色 / 深色、磨砂 /
清透、中英文、Increase Contrast、Reduce Motion、键盘与 VoiceOver 的完整
交叉矩阵仍不得推导为全部通过。
