...
| `QV-APPSTORE-CODEX-ISLAND-BRIDGE-001` | [Codex 灵动岛插件桥接实施规格](../design/quotaview-app-store-codex-island-plugin-bridge.md) | `Accepted` / `Implemented, Preview 7 Released` | App 沙盒只读桥已落地；支持用量快照的固定 `v1.0.0-preview.7` Pre-release、匿名 clone、隔离安装/卸载/重装和公开资产回下载复验均完成；灵动岛随付费 App 全量提供，不含 IAP 门禁 |
...
[Plugin Preview 7 Release](../release/PLUGIN_RELEASE_V1.0.0_PREVIEW.7.md)、
...
| `AS-SANDBOX-001` | `ENABLE_APP_SANDBOX = YES`、App Group、用户所选目录只读 bookmark、隐私清单；旧 CLI/Runtime/Helper/Socket 链路已删除 | 主 App 的 Network Client 已删除；网络和认证由官方 Codex 进程拥有；当前 Apple Development 签名 Build 4 Archive 已通过 Universal、签名与沙盒 entitlement 审计，最终 App Store Distribution Archive 仍须在其他外部门禁关闭后重建 |
