...
>
> 状态：`Accepted / Released`（重构、验证、公证、GitHub Latest 与 appcast 均已完成）
>
> 发布版本：`0.4.6 Build 1`，Sparkle 内部 Build `18`
>
> 用户已授权修复及版本身份，并在实机视觉确认后授权 GitHub 发布和 appcast。

后续用户发现任务未结束时出现完成回执，要求完整逻辑审计。发布曾暂停，
...
- 用户于 2026-09-05 确认视觉无问题并授权正式发布。签名、公证、Release 与
  appcast 的最终证据已记录于 [版本历史](../../VERSION_HISTORY.md#当前最新版本)，历史版本记录保留。
