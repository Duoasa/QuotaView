# QuotaView Handoff 历史快照 · 2026-09-12

这是整理前的交接记录，保留当时的验证、路径和未完成事项供追溯。
下文的“当前”“下一步”和授权均属于记录当时，不代表后续任务范围。
继续开发请使用 [当前 Handoff](../../HANDOFF.md)；发布事实使用
[版本历史](../../VERSION_HISTORY.md#当前最新版本)。
...
- [当前规格](../design/quotaview-codex-first-connection-0.4.8.md)：
  `QV-FIX-CODEX-FIRST-CONNECTION-019` / `Accepted / Released`。
  [链路审计](../design/quotaview-connection-audit-0.4.8.md) 保留反例和验证边界。

验证：221 项 Swift 测试，219 通过、2 项既有 opt-in 跳过、0 失败；PR 与发布提交
main CI 均通过；发布文档合并后的最终 main CI（`34638612768`）也已成功。
Universal、永久开发台、Developer ID、公证/Staple、公开包回下载、
...
本地证据位于 `dist/verification/build4/release/`（忽略目录，不提交账户快照或备份）：

- `release-manifest.json`：发布、签名、公证、Feed 与 CI 核验清单。
- `cleanup-result.json`、`install-result.json`、`installed-runtime-check.json`：
  调试副本清理、正式版安装、唯一 Widget 注册与有效共享快照检查。
- `final-main-ci.log`：最终 main CI 成功记录；`cleanup-backup/`：清理前的可恢复备份。

下一步先收集桌面小组件实际显示反馈，再记录真实 Intel 首次安装、系统权限交互、
长时间运行和完整视觉/辅助功能矩阵的验收结果；真实 N → N+1 自动更新安装仍待记录。
这些项目保持待验收，不能由自动化测试或正式版安装成功推定通过。
后续发布使用新的产品 Build 与递增内部序号；当前未发布修复见下节。
工作区 `/Users/sukduoasa/Documents/widget/.worktrees/QuotaView-0.4.8`；
Git 分支、HEAD 和远端状态需实时核实，不据历史记录判断。

## 当前迭代：特效长时间稳定性修复

[LONG-020 规格](../design/quotaview-effect-longevity.md)，已获实现授权。
...
并纳入 appcast。该次发布身份为 **0.4.7 Build 3 / internal 22 / v0.4.7-build.3**，
发布提交 `d185c9ae913c15a237ac2d223ed0a868f999f091`（PR #50）；完整资产和验证见顶部版本历史。
该次发布时 appcast 序列为 22 → 19 → 18。被撤回的 Build 2 已转为草稿，公开时间线不再显示，
...
- 修复完成态 27% 百分号丢失、单行浮点边界和中英文 / Emoji 长文字越界；
  [文字边界规格](../design/quotaview-activity-island-turn-token-usage-0.4.5.md#6-2026-09-11-百分号与文字边界修复)。
- 代理协议菜单靠右对齐、七个设置侧栏图标改为彩色圆角矩形；
  [代理规格](../design/quotaview-proxy-settings-0.4.7.md)。
...
当前修复见 [0.4.6 Token 兼容规格](../design/quotaview-codex-token-compatibility-0.4.6.md)。
用户曾确认单一四步任务的实机视觉并授权发布，随后发现主任务运行时出现错误
完成回执，要求审计整条逻辑链路。审计期间暂停了发布；现已修复并获本轮视觉确认，正式发布 0.4.6。
见 [0.4.6 灵动岛逻辑审计](../design/quotaview-activity-logic-audit-0.4.6.md)：
9 个诊断场景已复现，涉及内部任务准入、展示归属、turn 终态、重复开始与恢复。
已按审计完成系统重构；9 个诊断场景已转为永久回归，完整测试 166 项通过。
详见 [重构验证记录](../design/quotaview-activity-refactor-verification-0.4.6.md)。
...
当前独立实验：[量子噪点 Demo 控制台](../design/quotaview-quantum-motion-console.md)，
...
| Spec | 状态 | 结论 / 未完成项 |
|---|---|---|
| [`QV-PROTOTYPE-QUANTUM-MOTION-016`](../design/quotaview-quantum-motion-console.md) | `Accepted / Released` | 当前效果已获用户验收；177 项本地及 CI 测试、Universal、公证、回下载与 appcast 已验证，Build 2 正式发布 |
| [`QV-FIX-CODEX-TOKEN-COMPATIBILITY-015`](../design/quotaview-codex-token-compatibility-0.4.6.md) | `Accepted / Released` | 本地及 CI 166 项测试、Universal、真实五步视觉通过；PR #45 已合并，签名、公证、Latest、回下载与公开 appcast 验证完成 |
| [`QV-RELEASE-0.4.5-001`](../design/quotaview-0.4.5-release.md) | `Accepted / Released` | 130 项测试、Universal Developer ID、公证/Staple、GitHub Latest、回下载验证与 Stable appcast 在线 EdDSA 均已完成 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-CONFIRMATION-REMINDER-014`](../design/quotaview-activity-island-confirmation-reminder-0.4.5.md) | `Accepted / Released` | 真实等待确认持续满 10 秒后显示静态黄色描边与四周黄色光晕；已随 0.4.5 发布 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-TURN-TOKENS-013`](../design/quotaview-activity-island-turn-token-usage-0.4.5.md) | `Accepted / Released` | rollout 主通道的实时 Token、真实成功终态左右回执、当前额度与紧凑额度环已随 0.4.5 发布 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-HOVER-TRANSPARENCY-012`](../design/quotaview-activity-island-hover-transparency-0.4.5.md) | `Accepted / Released` | 悬停时整个灵动岛保留 20% 可见度且继续点击穿透；已随 0.4.5 发布 |
| [`QV-PRODUCT-CODEX-SOCKET-AUTOCONNECT-011`](../design/quotaview-codex-socket-autoconnect-0.4.5.md) | `Accepted / Released` | 只读本地任务流主通道、共享 Socket/Hook 回退及真实四步计划已验证并随 0.4.5 发布 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-CODEX-PLAN-010`](../design/quotaview-codex-plan-compatibility-0.4.4.md) | `Accepted / Superseded` | 0.4.4 未发布；原生计划、Goal、等待、真实终态和共享桥成果已由 0.4.5 继承 |
| [`QV-RELEASE-0.4.3-001`](../design/quotaview-0.4.3-release.md) | `Accepted / Released` | 104 项测试、Universal、Developer ID、公证/Staple、GitHub Latest、回下载启动与 Stable appcast 在线 EdDSA 均已完成 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-QUANTUM-NOISE-009`](../design/quotaview-quantum-noise-effect-correction.md) | `Accepted / Released` | 保留 `dropField` 持久化兼容；连续换色、增强闪灭、压缩上下文与完成反馈已随 0.4.3 发布 |
| [`QV-RELEASE-0.4.2-001`](../design/quotaview-0.4.2-release.md) | `Accepted / Released` | Developer ID、公证/Staple、GitHub Latest、回下载启动和 Stable appcast 在线 EdDSA 均已完成 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-LIFECYCLE-008`](../design/quotaview-activity-island-lifecycle-continuity-0.4.2.md) | `Accepted / Released` | 投递来源、ACK/队列、turn 感知终态锁与生命周期动画门控已实现；完成态只接受真实 `Stop` |
| [`QV-PRODUCT-ACTIVITY-ISLAND-PROGRESS-EFFECTS-007`](../design/quotaview-progress-effects-0.4.2.md) | `Accepted / Released` | 四种真实预览、细密 Drops、清晰 Slosh、状态配色适配和平滑完成反馈已随 0.4.2 发布 |
| [`QV-RELEASE-0.4.1-001`](../design/quotaview-0.4.1-release.md) | `Accepted / Released` | Developer ID、公证/Staple、GitHub Latest、回下载启动和 Stable appcast 在线签名核验均已完成 |
| [`QV-RELEASE-0.4.0-001`](../design/quotaview-0.4.0-development.md) | `Superseded / Released` | 0.4.0 只保留为大规模开发身份，不创建公开 Release；成果已由 0.4.1 Build 1 正式发布 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-STATE-SMOKE-006`](../design/quotaview-codex-activity-island-state-smoke-0.4.0.md) | `Accepted / Released` | 进度条顶层样式、计划识别、单步骤回退、完成高光分层与结束事件收敛已随 0.4.1 Build 1 发布 |
| [`QV-PRODUCT-ACTIVITY-ISLAND-SIZE-005`](../design/quotaview-codex-activity-island-size-0.4.0.md) | `Superseded / Released` | 0.4.1 已发布的 AI 球尺寸能力作为历史保留；0.4.5 已移除 AI 球及其展开尺寸选择器 |
| [`QV-PRODUCT-QUOTA-WINDOWS-003`](../design/quotaview-quota-windows-0.3.6-build.3.md) | `Accepted / Released` | 多周期额度已随 0.3.7 Build 1 发布并进入 Stable Feed |
| [`QV-PRODUCT-ACTIVITY-ISLAND-004`](../design/quotaview-codex-activity-island-0.3.6.md) | `Accepted / Released` | “锁定到 Codex 屏幕”已随 0.3.7 Build 1 发布；不包含多任务 Preview |
| [`QV-PRODUCT-APP-UPDATES-003`](../design/quotaview-app-updates-0.3.5.md) | `Accepted / Verifying` | 0.4.7 Build 3 已进入 Stable Feed，线上签名已验证；尚缺一次由旧版客户端发起的真实 N → N+1 替换与重启记录 |
...
- [长期执行与设计规范](../../AGENTS.md)
- [SDD 注册表](../specs/README.md)
- [SDD 开发流程](../specs/DEVELOPMENT_PROCESS.md)
