...
- 回下载或开发调试副本的运行验证结束后，关闭临时进程；在已有清理授权范围内，
  注销临时 App / Widget，保留可恢复归档后移除展开的副本。保留源码、永久开发台
  和发布 ZIP，避免同 Bundle ID 的临时包继续参与系统选择。
- 正式安装或自动更新后，核对 Widget 的有效注册路径与进程实际加载路径；
  进程命令行指向 `/Applications` 不代表其仍映射的二进制已更新。发现旧版本
  或已移除更新缓存的映射时，仅重启对应 Widget 进程并复查。
- 确认正式 Widget 唯一注册、共享快照有效且持续更新、时间线请求成功；
  实际额度画面由用户确认。不要为恢复显示而删除账户数据或共享快照。
